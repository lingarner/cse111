...
def main():
    ...
    parse_formula('H2O', periodic_table_dic)
...
def make_periodic_table():
    ...
    for elements in periodic_table_dic.items():
...
# symbol_quantity_list, periodic_table_dic
def compute_molar_mass():
    ...
    for symbol, quantity in periodic_table_dic.items():
        symbol = symbol[SYMBOL_INDEX]
        quantity = quantity[QUANTITY_INDEX]
    
    symbol_quantity_list = [
        [SYMBOL_INDEX, QUANTITY_INDEX]
    ]
    print(symbol_quantity_list)
    
    # for atomic_mass in periodic_table_dict.items():
    #     atomic_mass = atomic_mass[ATOMIC_MASS_INDEX]
    #     print(f'{symbol}: {atomic_mass}')

    # mol = atomic_mass * quantity
