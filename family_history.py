...
def print_death_age(people_dict):
    ...
    for person_key, death_list in people_dict.items():
        person_name = death_list[NAME_INDEX]
        person_age = death_list[DEATH_YEAR_INDEX] - death_list[BIRTH_YEAR_INDEX]
        print(f'{person_name}, {person_age}')
...
def count_genders(people_dict):
    ...
    male = []
    female = []
    for person_key, gender in people_dict.items():
        ...
        if gender == 'M':
            ...
            num_women = len(female)       
    print(f"There are {num_men} males and {num_women} females.")
pass
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage in marriages_dict.items():
        husbands = marriage[HUSBAND_KEY_INDEX]
        wives = marriage[WIFE_KEY_INDEX]
        wedding = marriage[WEDDING_YEAR_INDEX]

        husband_lists = people_dict[husbands]
    ...
    for person_key, marriages in marriages_dict.items():
        husband = marriages[HUSBAND_KEY_INDEX]
        wife = marriages[WIFE_KEY_INDEX]
        wedding = marriages[WEDDING_YEAR_INDEX]

        husband_key = people_dict[husband]
        husband_name = husband_key[NAME_INDEX]
        husband_age_one = husband_key[BIRTH_YEAR_INDEX] 
        husband_age_two =  wedding - husband_age_one

        wife_key = people_dict[wife]
        wife_name = wife_key[NAME_INDEX]
        wife_age_one = wife_key[BIRTH_YEAR_INDEX] 
        wife_age_two =  wedding - wife_age_one

        
    #     print(f"The husband is named {husband_name}, he is {husband_age_two} years old and was married in {wedding}.")
    #     print(f"The wife is named {wife_name}, she is {wife_age_two} years old and was married in {wedding}.")
